...
func TestDevicesCreateDryRunSkipsRealCall(t *testing.T) {
	called := false
	stdout, stderr, code := executeCommand(t, http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		called = true
	}), []string{"devices", "create", "--name=laptop", "--profile-id=p1", "--dry-run", "--format=json"})
	...
	if bytes.Contains(stdout, []byte(`"pk":""`)) {
		t.Fatalf("dry-run output must not emit a zero-valued pk: %s", stdout)
	}
...
func TestDevicesUpdateDryRunSkipsRealCall(t *testing.T) {
	called := false
	stdout, stderr, code := executeCommand(t, http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		called = true
	}), []string{"devices", "update", "--device-id=dev1", "--name=renamed", "--dry-run", "--format=json"})
